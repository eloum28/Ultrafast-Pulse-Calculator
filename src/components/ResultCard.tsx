...
export function ResultCard({ result, calculatedFields }: ResultCardProps) {
  if (!result) {
    return (
      <div className="bg-slate-900/30 border border-slate-800 rounded-xl p-8 text-center h-full flex flex-col items-center justify-center">
        <div className="text-slate-600 mb-2">
          <svg className="w-12 h-12 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
          </svg>
        </div>
        <h3 className="text-lg font-medium text-slate-300 tracking-tight">Ready to calculate</h3>
  ...
  const renderMath = (math: string) => {
    return (
      <div className="my-4 overflow-x-auto text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-950 p-4 rounded-lg shadow-inner border border-slate-100 dark:border-slate-800">
        <ReactMarkdown
...
$$E_{\\text{pulse}} = ${engEnergy.value.toPrecision(3)}\\text{ }${engEnergy.unit}$$
        `;
        return (
          <div key="energy" className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
...
$$P_{\\text{peak}} \\approx ${engPeak.value.toPrecision(3)}\\text{ }${engPeak.unit}$$
        `;
        return (
          <div key="peak" className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
            ...
            {pulseShape !== 'rectangular' && (
              <p className="text-[11px] text-amber-500/80 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20 mt-4 leading-relaxed">
...
$$P_{\\text{avg}} = ${engPower.value.toPrecision(3)}\\text{ }${engPower.unit}$$
        `;
        return (
          <div key="avg" className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          ...
          return (
            <div key={item.label} className={`bg-slate-900 border ${isCalculated ? 'border-cyan-500/50' : 'border-slate-800'} rounded-xl p-6 shadow-xl flex flex-col justify-between`}>
              <div>
                <span className="text-[10px] uppercase tracking-widest text-cyan-500 font-bold flex justify-between">
                  {isCalculated ? 'Calculated ' : ''}{item.label}
                </span>
                <div className="text-5xl font-light text-white mt-2">
                  {eng.value.toPrecision(3)} <span className="text-2xl text-cyan-400">{eng.unit}</span>
                </div>
                <div className="text-sm text-slate-500 mt-2 font-mono flex flex-col space-y-1">
                  ...
                  {result.uncertainties && result.uncertainties[item.label.replace(' ', '').charAt(0).toLowerCase() + item.label.replace(' ', '').slice(1) as 'pulseEnergy'|'peakPower'] > 0 && isCalculated && (
                    <span className="text-amber-500/80">± {result.uncertainties[item.label.replace(' ', '').charAt(0).toLowerCase() + item.label.replace(' ', '').slice(1) as 'pulseEnergy'|'peakPower'].toPrecision(3)}%</span>
                  )}
                </div>
              </div>
              <div className="mt-4 p-3 bg-slate-950/50 rounded-lg font-mono text-[11px] text-slate-400">
                {item.formula}
              </div>
            </div>
          );
      ...
      {/* Duty Cycle Info */}
      <div className="flex flex-col sm:flex-row bg-slate-900 border border-slate-800 rounded-xl overflow-hidden mt-6">
        <div className="sm:w-1/4 p-4 border-b sm:border-b-0 sm:border-r border-slate-800">
          <span className="block text-[10px] text-slate-500 uppercase font-bold">Duty Cycle %</span>
          <span className="text-xl font-bold text-white">{engDutyCycle} %</span>
        </div>
        <div className="sm:w-1/4 p-4 border-b sm:border-b-0 sm:border-r border-slate-800">
          <span className="block text-[10px] text-slate-500 uppercase font-bold">Repetition Period</span>
          <span className="text-xl font-bold text-white">{formatEngineering(result.repetitionPeriod, TIME_UNITS).value.toPrecision(3)} {formatEngineering(result.repetitionPeriod, TIME_UNITS).unit}</span>
        </div>
        <div className="sm:w-2/4 p-4 flex items-center bg-slate-900/50">
      ...
      <div className="mt-4 p-3 rounded bg-amber-500/10 border border-amber-500/20 text-[11px] text-amber-200 leading-relaxed">
      ...
      <details className="mt-6 group bg-slate-900 border border-slate-800 rounded-xl overflow-hidden [&_summary::-webkit-details-marker]:hidden">
        <summary className="flex items-center justify-between p-4 cursor-pointer hover:bg-slate-800/50 transition-colors text-slate-300 select-none">
          <span className="text-xs font-bold uppercase tracking-widest text-cyan-500">Detailed Calculations</span>
          <svg className="w-4 h-4 transition-transform group-open:rotate-180 text-cyan-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </summary>
        <div className="p-4 border-t border-slate-800 bg-slate-950/50 space-y-4 text-xs text-slate-400">
          <div className="space-y-2">
            <h4 className="text-cyan-400 font-bold uppercase tracking-widest text-[10px]">Variables</h4>
            <div className="prose prose-sm prose-invert max-w-none prose-p:my-1 prose-math:text-cyan-400">
...
* $K_{shape}$ = Shape Factor = ${pulseShape === 'rectangular' ? '1' : pulseShape === 'gaussian' ? '~0.94' : '~0.88'}`}
          ...
          <div className="space-y-2">
            <h4 className="text-cyan-400 font-bold uppercase tracking-widest text-[10px]">Formulas & Substitution</h4>
            <div className="prose prose-sm prose-invert max-w-none prose-p:my-1 prose-math:text-cyan-400">
              <ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]}>
...
\`\`\``}
          ...
          <div className="space-y-2">
            <h4 className="text-cyan-400 font-bold uppercase tracking-widest text-[10px]">Physical Interpretation</h4>
